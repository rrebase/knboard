...
def test_delete_comment_not_authenticated(api_client, task_factory, comment_factory, steve):
    task = task_factory()
    task.column.board.members.set([steve])
    comment = comment_factory(task=task, author=steve)
...
def test_delete_comment_not_author(api_client, task_factory, comment_factory, steve, amy):
    task = task_factory()
